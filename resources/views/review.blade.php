...
@section('content')
    <section class="py-8 antialiased dark:bg-gray-900 md:py-16 min-h-screen bg-gradient-to-br from-white to-blue-100 dark:from-gray-900 dark:to-gray-800">
        <form method="POST" action="{{ route('review.store') }}" class="mx-auto px-4 2xl:px-0 flex gap-4 justify-center">
            @csrf
            <input type="hidden" name="id_product" value="{{ $product->id }}">
            <div class="space-y-6 bg-white dark:bg-gray-800 p-8 w-full md:w-3/4 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700" x-data="{ selectedRating: 0 }">
                <h4 class="text-2xl font-bold text-gray-900 dark:text-white mb-4">📝 Submit Your Review</h4>
