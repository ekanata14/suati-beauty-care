...
<section class="py-12 bg-gradient-to-b from-white to-blue-100 min-h-screen">
    <div class="container mx-auto px-4 flex flex-col lg:flex-row gap-6">
        @if ($cart)
        <form action="{{ route('checkout.multiple') }}" method="POST" id="checkout-form" class="flex flex-col lg:flex-row gap-6 w-full">
            @csrf

            <!-- Cart Items -->
            <div class="bg-white rounded-xl shadow-md w-full lg:w-3/4 p-6" data-aos="fade-right" data-aos-duration="500">
                ...
                <div class="overflow-x-auto border-b border-gray-200 dark:border-gray-700">
                    <table class="min-w-full text-sm text-left text-gray-700 dark:text-white">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                            ...
                            <tr>
                                <td class="p-3">
                                    <input type="checkbox" class="select-item" data-id="{{ $item->produk->id }}" data-harga="{{ $item->produk->harga }}" data-qty="{{ $item->qty }}">
                                </td>
                                <td class="p-3">
                                    <div class="flex items-center gap-4">
                                        <img src="{{ asset('storage/' . $item->produk->foto_produk) }}" alt="{{ $item->produk->nama }}" class="w-12 h-12 object-cover rounded" />
                                        <div>
                                            <p class="font-semibold">{{ $item->produk->nama }}</p>
                                            <p class="text-xs text-gray-500">IDR {{ number_format($item->produk->harga, 0, ',', '.') }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-3">{{ $item->qty }}x</td>
                                <td class="p-3">IDR {{ number_format($item->produk->harga * $item->qty, 0, ',', '.') }}</td>
                                <td class="p-3">
                                    <a href="{{ route('cart.delete', $item->id) }}" class="text-red-600 hover:underline delete-link">Hapus</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
            ...
            <!-- Summary -->
            <div class="bg-white rounded-xl shadow-md w-full lg:w-1/4 p-6" data-aos="fade-left" data-aos-duration="500">
                ...
                <div class="mt-6 flex flex-col gap-3">
                    <button type="submit" onclick="return confirmCheckout(event)" class="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700">Checkout</button>
                    <a href="{{ route('products') }}" class="text-center text-sm border border-gray-300 py-2 rounded hover:bg-gray-100">Kembali Belanja</a>
                </div>
            </div>
        </form>
                ...
                function updateTotals() {
                    ...
                    checkboxes.forEach(cb => {
                        if (cb.checked) {
                            ...
                            totalQty += qty;
                            selectedItems.push({ id, qty, harga: harga * qty });
                        }
                    });
